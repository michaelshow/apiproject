...
func (m *Member) Login(account string, password string) (*Member, error) {
	...
	if err != nil {
		if beego.AppConfig.DefaultBool("ldap_enable", false) {
			logs.Info("转入LDAP登陆 ->", account)
			return member.ldapLogin(account, password)
		} else if url := beego.AppConfig.String("http_login_url"); url != "" {
...
func (m *Member) ldapLogin(account string, password string) (*Member, error) {
	...
	var err error
	ldaphost := beego.AppConfig.String("ldap_host")
	lc, err := ldap.Dial("tcp", fmt.Sprintf("%s:%d", ldaphost, beego.AppConfig.DefaultInt("ldap_port", 3268)))
	if err != nil {
		logs.Error("绑定 LDAP 用户失败 ->", err)
		return m, ErrLDAPConnect
	}
	defer lc.Close()
	ldapuser := beego.AppConfig.String("ldap_user")
	ldappass := beego.AppConfig.String("ldap_password")
	err = lc.Bind(ldapuser, ldappass)
	if err != nil {
		logs.Error("绑定 LDAP 用户失败 ->", err)
		return m, ErrLDAPFirstBind
	}
	ldapbase := beego.AppConfig.String("ldap_base")
	ldapfilter := beego.AppConfig.String("ldap_filter")
	ldapattr := beego.AppConfig.String("ldap_attribute")
